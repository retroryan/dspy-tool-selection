...
def validate_args(args: Dict[str, Any], validators: List[FieldValidator]) -> Dict[str, Any]:
    """
    ...
    for validator in validators:
        ...
        try:
            ...
            if not _validate_constraints(parsed_value, validator):
                error_msg = validator.error_message or f"Invalid value for '{validator.name}'"
                return {"error": error_msg}
                
            validated[validator.name] = parsed_value
            
        except (ValueError, TypeError) as e:
            error_msg = validator.error_message or f"Invalid {validator.field_type.value} for '{validator.name}'"
            return {"error": error_msg}
...
def optional_string(name: str, default: str = "", **kwargs) -> FieldValidator:
    ...
    return FieldValidator(name, FieldType.STRING, required=False, default=default, **kwargs)
...
def optional_int(name: str, default: int = 0, **kwargs) -> FieldValidator:
    ...
    return FieldValidator(name, FieldType.INTEGER, required=False, default=default, **kwargs)
